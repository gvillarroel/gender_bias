...
def to_text_only_she(row):
    return f"{row.phrase}".replace("{occupation}", f"The {row.Role}").replace("[MASK]", "she")

def to_text_mid(row):
    return f"{row.phrase}".replace("{occupation}", f"The {row.Role}").replace("[MASK]", np.random.choice(["she", "he"]))

def to_text_invert(row):
    return f"{row.phrase}".replace("{occupation}", f"The {row.Role}").replace("[MASK]", np.random.choice(["she", "he"], p=[row.he_prob, row.she_prob]))
...
for index in range(3):
    ...
    for (name, transform) in [("she", to_text_only_she,), ("mid", to_text_mid,), ("invert", to_text_invert,)]:
